...
import { ArrowUpDown, Search, X, MoreHorizontal, Trash2, Download } from 'lucide-react';
...
const STATUS_STYLES: Record<string, string> = {
  ...
  listed: 'bg-amber-500/15 text-amber-400 border-amber-500/25',
  ...
  const columns: ColumnDef<InventoryItem>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Name',
        cell: ({ getValue }) => (
          <span className="text-zinc-50 text-sm truncate max-w-[200px] block">{getValue<string>()}</span>
        ),
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span className="text-xs text-zinc-400 capitalize">{cat === 'funko' ? 'Funko Pops' : cat}</span>
          );
        },
      },
      {
        ...
        header: 'Purchase',
        cell: ({ getValue }) => (
          <span className="font-mono tabular-nums text-zinc-300">{formatCurrency(getValue<number>())}</span>
        ),
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span className="font-mono tabular-nums text-zinc-300">{v != null ? formatCurrency(v) : '-'}</span>
          );
        },
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span
              className={cn('font-mono tabular-nums', v >= 0 ? 'text-green-400' : 'text-red-400')}
            >
          ...
          return (
            <span
              className={cn(
                ...
                status !== 'sold' && status !== 'returned' && days >= 60 && 'text-red-400',
                status !== 'sold' && status !== 'returned' && days >= 30 && days < 60 && 'text-amber-400',
                (status === 'sold' || status === 'returned' || days < 30) && 'text-zinc-400'
              )}
            >
              {days}d
            </span>
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="relative">
              ...
              {isOpen && (
                <div className="absolute right-0 top-full mt-1 z-50 w-40 rounded-md border border-zinc-800 bg-zinc-900 py-1 shadow-lg">
                  {item.status === 'in_stock' && (
                    <>
                      <button
                        onClick={() => { onMarkAsListed(item); setOpenMenu(null); }}
                        className="w-full px-3 py-1.5 text-left text-xs text-zinc-300 hover:bg-zinc-800"
                      >
                        Mark as Listed
                      </button>
                      <button
                        onClick={() => { onMarkAsSold(item); setOpenMenu(null); }}
                        className="w-full px-3 py-1.5 text-left text-xs text-zinc-300 hover:bg-zinc-800"
                      >
                        Mark as Sold
                      </button>
                  ...
                  <button
                    onClick={() => { onDelete([item.id]); setOpenMenu(null); }}
        ...
        {selectedIds.length > 0 && (
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-md border border-zinc-800 px-2 py-1 text-xs text-red-400 hover:bg-zinc-900 transition-colors"
            >
              ...
              <tr key={hg.id} className="border-b border-zinc-800 bg-zinc-900/50">
                {hg.headers.map((header) => (
                  <th
                    key={header.id}
                    className={cn(
                      'px-3 py-2 text-left text-xs font-medium text-zinc-500 select-none transition-colors',
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      {header.column.getIsSorted() && <ArrowUpDown className="h-3 w-3 text-amber-500" />}
                    </div>
                  </th>
                ))}
              </tr>
