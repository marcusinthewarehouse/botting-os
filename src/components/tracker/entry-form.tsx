...
export function EntryForm({
  ...
  const [category, setCategory] = useState(
    initialValues?.category ?? "sneakers",
  );
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent>
        <SheetHeader>
          ...
          <SheetDescription>
            Record a purchase, sale, cancel, or refund.
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-6">
          <div className="space-y-2">
            ...
            <select
              ...
              className="flex h-9 w-full rounded-md border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50"
            >
              {TYPES.map((t) => (
