...
import { PricingService, type PriceData, type ProductResult, type SizePrice } from '@/services/pricing';
...
function getSizePrice(sizes: SizePrice[], selectedSize: string): { ask: number; bid: number } {
  if (!selectedSize || sizes.length === 0) return { ask: 0, bid: 0 };
  const match = sizes.find((s) => s.size === selectedSize);
  return match ? { ask: match.lowest_ask, bid: match.highest_bid } : { ask: 0, bid: 0 };
}
...
export function PriceComparison({
  ...
  const [stockx, setStockx] = useState<MarketplaceData>({ priceData: null, loading: false, override: '' });
  const [goat, setGoat] = useState<MarketplaceData>({ priceData: null, loading: false, override: '' });
  const [ebayPrice, setEbayPrice] = useState<{ median: number; loading: boolean; override: string }>({
    median: 0,
    loading: false,
    override: '',
  });
  ...
  useEffect(() => {
    ...
    const goatAsk = parseFloat(goat.override) || (goat.priceData?.lowest_ask ?? 0);
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {sizes.map((s) => (
              <button
                key={s.size}
                onClick={() => onSizeChange(s.size === selectedSize ? '' : s.size)}
                className={cn(
...
function MarketplaceCard({
  ...
  if (loading) {
    return (
      <Card className="p-4 bg-black border-white/[0.06] space-y-3">
  ...
  const sizeData = selectedSize ? getSizePrice(priceData?.sizes ?? [], selectedSize) : null;
    ...
    <Card className="p-4 bg-black border-white/[0.06] space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={onRefresh} className="p-1 rounded hover:bg-zinc-800">
        ...
        <>
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-500">Lowest Ask</span>
            ...
            {lastSale > 0 && (
              <div className="flex justify-between text-xs">
                ...
                <span className="font-mono tabular-nums text-zinc-200">${lastSale.toFixed(2)}</span>
              </div>
            )}
          </div>

          <div className="pt-2 border-t border-white/[0.06]">
            <Label className="text-zinc-500 text-[10px] uppercase tracking-wider">Override</Label>
            <div className="relative mt-1">
              <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-zinc-600">$</span>
              <Input
                type="number"
                min="0"
                step="0.01"
                ...
                className="h-7 bg-zinc-900 border-zinc-800 pl-5 text-xs font-mono tabular-nums"
              />
            </div>
          </div>
...
function EbayCard({
  ...
  return (
    <Card className="p-4 bg-black border-white/[0.06] space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-zinc-50">eBay</p>
        <button onClick={onRefresh} className="p-1 rounded hover:bg-zinc-800">
          <RefreshCw className="size-3 text-zinc-500" />
        </button>
      </div>
