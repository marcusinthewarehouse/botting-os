...
export function PriceAlertDialog({
  ...
  }, [targetPrice, styleId, productName, selectedMarketplace, size, direction, currentPrice, recurring, imageUrl, loadAlerts]);
        ...
        <div className="space-y-6 mt-6">
          ...
          {styleId && (
            <Card className="p-4 bg-zinc-900 border-zinc-800 space-y-4">
              ...
              {productName && (
                <p className="text-sm text-zinc-200 truncate">{productName}</p>
              )}

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-zinc-500 text-xs">Marketplace</Label>
                  <select
                ...
                <div className="space-y-1.5">
                  <Label className="text-zinc-500 text-xs">Direction</Label>
                  <select
                    value={direction}
              ...
              <div className="space-y-1.5">
                <Label className="text-zinc-500 text-xs">Target Price</Label>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="alert-recurring" className="text-zinc-400 text-xs">
...
function AlertRow({
  ...
  return (
    <Card className="flex items-center justify-between px-3 py-2.5 bg-black border-white/[0.06] group">
      <div className="min-w-0 flex-1">
        <p className="text-xs text-zinc-200 truncate">{alert.productName}</p>
        <div className="flex items-center gap-2 text-[10px] text-zinc-500 mt-0.5">
          <span>{MARKETPLACE_LABELS[alert.marketplace] ?? alert.marketplace}</span>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {alert.recurring && (
          <Badge variant="outline" className="text-[10px] bg-amber-500/15 text-amber-400 border-amber-500/25">
            recurring
          </Badge>
        )}
        <button
          onClick={() => onDelete(alert.id)}
          className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-zinc-800 transition-all duration-150"
        >
