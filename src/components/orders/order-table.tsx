...
const columns: ColumnDef<CheckoutEvent>[] = [
      ...
      return (
        <span
          className={cn(
            ...
            BOT_COLORS[bot] ?? BOT_COLORS.unknown,
          )}
        >
          {bot}
        </span>
  ...
  },
  {
    ...
    header: "Product",
    cell: ({ getValue }) => (
      <span className="text-foreground text-sm truncate max-w-[240px] block">
        {getValue<string>()}
      </span>
    ),
  },
...
export function OrderTable({ events, loading }: OrderTableProps) {
  ...
  const botFilter =
    (columnFilters.find((f) => f.id === "bot_name")?.value as string) ?? "";
  const storeFilter =
    (columnFilters.find((f) => f.id === "store")?.value as string) ?? "";
  const statusFilter =
    (columnFilters.find((f) => f.id === "success")?.value as string) ?? "";

  const hasActiveFilters =
    globalFilter || botFilter || storeFilter || statusFilter;
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <select
          ...
          className="rounded-md border border-border bg-card px-3 py-2 text-sm text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary/50"
        >
        ...
        {hasActiveFilters && (
          <button
            ...
            className="flex items-center gap-1 rounded-md px-2 py-2 text-xs text-muted-foreground hover:text-foreground/80 transition-colors"
          >
            <X className="h-3 w-3" />
            Clear
          </button>
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1 rounded-md border border-border bg-card px-3 py-2 text-xs text-muted-foreground hover:text-foreground/80 transition-colors"
            >
              <Columns3 className="h-3.5 w-3.5" />
              Columns
            </button>
            {showColumnPicker && (
              <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-md border border-border bg-card p-2 shadow-lg">
                {table.getAllLeafColumns().map((column) => (
                  <label
                    key={column.id}
                    className="flex items-center gap-2 rounded px-2 py-1 text-xs text-muted-foreground hover:bg-muted cursor-pointer"
                  >
                    <input
                      type="checkbox"
            ...
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id} className="border-b border-border bg-card/50">
                {hg.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-3 py-2 text-left text-xs font-medium text-muted-foreground cursor-pointer select-none hover:text-muted-foreground transition-colors"
            ...
            {table.getRowModel().rows.length === 0 ? (
              ...
              table.getRowModel().rows.map((row) => (
                <>
                  ...
                  {expandedRow === row.id && (
                    <tr key={`${row.id}-detail`} className="bg-card/30">
                      <td
                        colSpan={table.getVisibleLeafColumns().length}
                        className="px-4 py-3"
                      >
...
function ExpandedDetail({ event }: { event: CheckoutEvent }) {
  const details = [
    ...
    {
      label: "Quantity",
      value: event.quantity > 1 ? String(event.quantity) : null,
    },
  ].filter((d) => d.value);
  ...
  return (
    <div className="space-y-3">
      ...
      {extraEntries.length > 0 && (
        <div>
          <div className="text-xs text-muted-foreground mb-1">Extra fields</div>
