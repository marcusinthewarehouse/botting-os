...
export default function InventoryPage() {
  ...
  const [categories, setCategories] = useState<{ category: string; count: number; totalValue: number }[]>([]);
  ...
  const handleAddItem = useCallback(async (data: AddItemData) => {
    try {
      const { inventoryRepo } = await import('@/lib/db/repositories');
  ...
  const handleMarkAsListed = useCallback(async (item: InventoryItem) => {
    try {
      const { inventoryRepo } = await import('@/lib/db/repositories');
  ...
  const handleConfirmSold = useCallback(async (soldPrice: number, marketplace: string) => {
    if (!soldItem) return;
    try {
      const { inventoryRepo } = await import('@/lib/db/repositories');
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-md bg-amber-500 px-3 py-1.5 text-sm font-medium text-zinc-950 hover:bg-amber-400 transition-colors"
        >
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <Boxes className="h-12 w-12 text-zinc-700 mb-4" />
          <h3 className="text-sm font-medium text-zinc-400">No inventory items</h3>
          <p className="text-xs text-zinc-600 mt-1 max-w-xs">
            Add items manually or they will auto-populate from successful checkouts.
          </p>
          <button
            onClick={() => setAddFormOpen(true)}
