...
export default function VccPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const withEmails: VccWithEmail[] = allVccs.map((vcc) => ({
      ...vcc,
      linkedEmail: vcc.linkedAccountId
        ? emailMap.get(vcc.linkedAccountId)
        : undefined,
    }));
    setVccs(withEmails);
  }, []);
  ...
  const selectClass =
    "rounded-md border border-border bg-card px-3 py-2 text-sm text-foreground/80 outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50";
