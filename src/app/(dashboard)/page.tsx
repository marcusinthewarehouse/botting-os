...
import { DollarSign, Mail, CreditCard, Boxes } from "lucide-react";
  ...
  return (
    <PageTransition>
      ...
      {!loaded ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <KpiCard
            ...
            icon={<DollarSign className="size-[14px] text-primary" />}
            iconColor="green"
            trendDirection={
              s && s.totalProfit > 0
                ? "up"
          ...
          <KpiCard
            ...
            icon={<Mail className="size-[14px] text-[#4d9eff]" />}
            iconColor="blue"
          />
          <KpiCard
            title="VCCs Tracked"
            value={s ? String(s.vccsTracked) : "0"}
            icon={<CreditCard className="size-[14px] text-[#ffb224]" />}
            iconColor="amber"
          />
          <KpiCard
            title="Inventory Items"
            value={s ? String(s.inventoryItems) : "0"}
            icon={<Boxes className="size-[14px] text-[#4d9eff]" />}
            iconColor="blue"
          />
        </div>
      )}
      ...
      {s &&
        ...
        s.inventoryItems === 0 && (
          <div
            className="mt-8 rounded-[10px] border border-border p-8 text-center"
            style={{ background: "#1e1e26" }}
          >
            <p className="text-sm text-muted-foreground">
              Add data to see your stats come alive
            </p>
            <p className="text-xs text-[#585870] mt-1">
              Press{" "}
              <kbd className="mx-1 rounded-md bg-[#25252e] border border-border px-1.5 py-0.5 text-xs text-muted-foreground">
                Cmd+K
              </kbd>{" "}
              to navigate
            </p>
