...
export async function getAnalytics(range: TimeRange): Promise<AnalyticsData> {
  ...
  try {
    ...
    const trackerInRange = allTracker.filter((e) => isInRange(e.date, rangeStart));
    const trackerInPrev = prevStart && rangeStart
      ? allTracker.filter((e) => e.date && e.date >= prevStart && e.date < rangeStart)
      : [];
    ...
    const ordersInRange = allOrders.filter((e) => isInRange(e.createdAt, rangeStart));
    ...
    for (const entry of trackerInRange) {
      ...
      if (entry.type === 'sale' || entry.type === 'refund') totalSaleRevenue += entry.amount;
    }
    ...
    for (const entry of trackerInPrev) {
      ...
      if (entry.type === 'sale' || entry.type === 'refund') prevRevenue += entry.amount;
    }
    const prevProfit = prevRevenue - prevSpend;
    const profitTrend = prevProfit !== 0 ? ((totalProfit - prevProfit) / Math.abs(prevProfit)) * 100 : 0;
    ...
    const catMap = new Map<string, { totalROI: number; totalProfit: number; count: number }>();
    for (const item of soldItems) {
      const cat = item.category ?? 'Other';
      const existing = catMap.get(cat) ?? { totalROI: 0, totalProfit: 0, count: 0 };
      const itemProfit = (item.soldPrice ?? 0) - item.purchasePrice;
      const itemROI = item.purchasePrice > 0 ? (itemProfit / item.purchasePrice) * 100 : 0;
    ...
    const categoryROI = Array.from(catMap.entries())
      .map(([category, data]) => ({
        category,
        roi: data.count > 0 ? Math.round((data.totalROI / data.count) * 10) / 10 : 0,
