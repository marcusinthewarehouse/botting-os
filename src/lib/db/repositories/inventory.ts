...
export async function markAsSold(id: number, soldPrice: number, marketplace?: string): Promise<void> {
...
export async function markAsListed(id: number, marketplace: string, listedPrice?: number): Promise<void> {
  await db.update(inventoryItems).set({
    status: 'listed',
...
export async function getCategoryCounts(): Promise<{ category: string; count: number; totalValue: number }[]> {
  const items = await db.select().from(inventoryItems).where(ne(inventoryItems.status, 'sold'));
