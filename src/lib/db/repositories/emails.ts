...
export async function getByIcloudAccount(account: string): Promise<Email[]> {
  return db
    .select()
    .from(emails)
    .where(eq(emails.icloudAccount, account))
...
export async function search(query: string): Promise<Email[]> {
  return db
    .select()
    .from(emails)
    .where(like(emails.address, `%${query}%`))
    .orderBy(desc(emails.createdAt));
}

export async function bulkCreate(
  items: Omit<NewEmail, "id" | "createdAt" | "updatedAt">[],
): Promise<void> {
  const now = new Date();
...
export async function addRetailerTag(
  id: number,
  retailer: string,
): Promise<void> {
  ...
  await db
    .update(emails)
    .set({ retailers: [...current, retailer], updatedAt: new Date() })
    .where(eq(emails.id, id));
}

export async function removeRetailerTag(
  id: number,
  retailer: string,
): Promise<void> {
  const row = await getById(id);
  ...
  await db
    .update(emails)
    .set({
      retailers: current.filter((r) => r !== retailer),
      updatedAt: new Date(),
    })
    .where(eq(emails.id, id));
}
