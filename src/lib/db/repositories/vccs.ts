...
export async function create(
  data: Omit<NewVcc, "id" | "createdAt" | "updatedAt">,
): Promise<void> {
  const now = new Date();
...
export async function update(
  id: number,
  data: Partial<Omit<NewVcc, "id" | "createdAt">>,
): Promise<void> {
  await db
    .update(vccs)
...
export async function getByProvider(provider: string): Promise<Vcc[]> {
  return db
    .select()
    .from(vccs)
    .where(eq(vccs.provider, provider))
    .orderBy(desc(vccs.createdAt));
}
...
export async function getByLinkedAccount(accountId: number): Promise<Vcc[]> {
  return db
    .select()
    .from(vccs)
    .where(eq(vccs.linkedAccountId, accountId))
    .orderBy(desc(vccs.createdAt));
}

export async function linkToAccount(
  vccId: number,
  accountId: number,
): Promise<void> {
  await db
