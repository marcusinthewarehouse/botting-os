...
type DecryptedVaultEntry = Omit<VaultEntry, 'passwordCiphertext' | 'passwordIv' | 'notesCiphertext' | 'notesIv'> & {
  password: string;
  notes: string | null;
};
...
async function decryptEntry(entry: VaultEntry): Promise<DecryptedVaultEntry> {
  ...
  const { passwordCiphertext: _pc, passwordIv: _pi, notesCiphertext: _nc, notesIv: _ni, ...rest } = entry;
  return { ...rest, password, notes };
}

export async function getAll(): Promise<DecryptedVaultEntry[]> {
  assertUnlocked();
  const rows = await db.select().from(vaultEntries).orderBy(desc(vaultEntries.createdAt));
  return Promise.all(rows.map(decryptEntry));
}

export async function getById(id: number): Promise<DecryptedVaultEntry | undefined> {
  assertUnlocked();
  const rows = await db.select().from(vaultEntries).where(eq(vaultEntries.id, id));
  if (!rows[0]) return undefined;
  return decryptEntry(rows[0]);
}

export async function create(data: { site: string; username: string; password: string; notes?: string }): Promise<void> {
  assertUnlocked();
  const { data: passwordCiphertext, iv: passwordIv } = await cryptoService.encrypt(data.password);
...
export async function update(
  id: number,
  data: Partial<{ site: string; username: string; password: string; notes: string }>
): Promise<void> {
  assertUnlocked();
