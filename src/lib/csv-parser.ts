...
export function detectFormat(headers: string[]): BankFormat {
  ...
  if (h.includes('transaction date') && h.includes('type') && h.includes('amount') && h.includes('memo')) {
    return 'chase';
  }
  ...
  if (h.length <= 4 && h.includes('date') && h.includes('description') && h.includes('amount')) {
    return 'amex';
  }
  return 'unknown';
}
...
export function parseTransactions(
  ...
  if (detectedFormat === 'chase') {
    const dateCol = findHeader(headers, 'Transaction Date') ?? 'Transaction Date';
