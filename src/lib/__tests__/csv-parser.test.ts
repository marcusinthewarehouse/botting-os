...
describe("detectFormat", () => {
  it("detects Chase format", () => {
    expect(detectFormat(["Transaction Date", "Type", "Amount", "Memo"])).toBe(
      "chase",
    );
  });
  ...
  it("detects Citi format", () => {
    expect(
      detectFormat(["Date", "Description", "Status", "Debit", "Credit"]),
    ).toBe("citi");
  });
...
describe("parseTransactions", () => {
  it("parses Chase CSV correctly", () => {
    const csv =
      "Transaction Date,Description,Amount,Category,Type,Memo\n01/15/2025,Nike Store,-150.00,Shopping,Sale,";
