...
describe("calculateEbayFees", () => {
  it("sneakers >= $100 no store: 8% + $0.40", () => {
    const result = calculateEbayFees(200, {
      category: "sneakers",
      hasStore: false,
    });
    expect(result.totalFees).toBe(16.4);
    expect(result.fees[0].rate).toBe(0.08);
    expect(result.fees[0].amount).toBe(16);
    expect(result.fees[1].amount).toBe(0.4);
    expect(result.netPayout).toBe(183.6);
  });

  it("sneakers < $100 no store: 12.55% + $0.40", () => {
    const result = calculateEbayFees(80, {
      category: "sneakers",
      hasStore: false,
    });
  ...
  it("sneakers < $10 no store: per-order fee is $0.30", () => {
    const result = calculateEbayFees(8, {
      category: "sneakers",
      hasStore: false,
    });
    expect(result.fees[1].amount).toBe(0.3);
  });
  ...
  it("sneakers >= $100 with store: 7% + $0.40", () => {
    ...
    expect(result.totalFees).toBe(14.4);
  });
  ...
  it("general category <= $7500: 13.25% + $0.40", () => {
    const result = calculateEbayFees(500, {
      category: "general",
      hasStore: false,
    });
  ...
  it("general category > $7500: tiered rate", () => {
    ...
    expect(result.fees[2].amount).toBe(0.4);
    expect(result.totalFees).toBe(1052.9);
  });
  ...
  it("books_media category: 15.3%", () => {
    ...
    expect(result.fees[0].amount).toBe(15.3);
    expect(result.totalFees).toBe(15.7);
  });
  ...
  it("very high price ($50,000) sneakers calculates correctly", () => {
    const result = calculateEbayFees(50000, {
      category: "sneakers",
      hasStore: false,
    });
    expect(result.fees[0].amount).toBe(4000);
    expect(result.totalFees).toBe(4000.4);
  });
});
...
describe("calculateGoatFees", () => {
  ...
  it("rating >= 90: 9.5% + $5 seller fee + 2.9% cashout", () => {
    ...
    expect(result.fees[2].amount).toBe(5.1);
    expect(result.totalFees).toBe(29.1);
    expect(result.netPayout).toBe(170.9);
  });

  it("rating 75: 15% commission", () => {
    const result = calculateGoatFees(200, {
      sellerRating: 75,
      isCanadian: false,
    });
  ...
  it("rating 60: 20% commission", () => {
    ...
    expect(result.fees[0].rate).toBe(0.2);
  ...
  it("canadian seller: +2.9% surcharge on commission", () => {
    ...
    expect(result.fees[0].amount).toBe(24.8); // 12.4% of 200
  });
  ...
  it("effectiveRate is calculated correctly", () => {
    const result = calculateGoatFees(200, baseOpts);
    expect(result.effectiveRate).toBeCloseTo(29.1 / 200, 4);
  });
...
describe("calculateFees", () => {
  ...
  it("routes to goat calculator", () => {
    const result = calculateFees(200, "goat", {
      goat: { sellerRating: 95, isCanadian: false },
    });
