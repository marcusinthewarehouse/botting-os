...
describe("detectBot edge cases", () => {
  ...
  it("handles embed with no footer", () => {
    expect(
      detectBot({ username: "RandomUser", embeds: [{ title: "Test" }] }),
    ).toBe("unknown");
  });

  it("handles embed with empty footer text", () => {
    expect(
      detectBot({ username: "RandomUser", embeds: [{ footer: { text: "" } }] }),
    ).toBe("unknown");
  });
  ...
  it("handles undefined username", () => {
    expect(
      detectBot({ embeds: [{ footer: { text: "Cybersole | v1.0" } }] }),
    ).toBe("cybersole");
  });
});
...
describe("normalizeFields edge cases", () => {
  ...
  it("handles fields with special characters in values", () => {
    const result = normalizeFields([
      {
        name: "Product",
        value: "Nike <script>alert('xss')</script>",
        inline: true,
      },
      ...
      embeds: [
        {
          ...
          fields: [
            ...
            {
              name: "Store",
              value: '<a href="javascript:void(0)">Click</a>',
              inline: true,
            },
          ],
        },
      ...
      embeds: [
        {
          ...
          fields: [
            {
              name: "Product",
              value: 'Nike Dunk Low "Panda" Edition',
              inline: true,
            },
  ...
  it("handles very long field values", () => {
    ...
    const payload: DiscordWebhookPayload = {
      ...
      embeds: [
        {
          title: "Checkout",
          fields: [{ name: "Product", value: longProduct, inline: true }],
        },
      ],
    };
    const event = parseWebhook(payload, userId);
