...
CogniLayer runs on a shared VPS. It CANNOT detect your project from CWD.
You MUST pass project=YOUR_PROJECT_NAME (from Project DNA section below) on EVERY call to:
memory_search, memory_write, file_search, project_context, session_bridge.
Example: memory_search(query="auth", project="my-app")
Without this, facts get saved to the wrong project or searches return nothing.
...
Files: src/app/layout.tsx (edit), src/components/titlebar.tsx (edit), src-tauri/src/lib.rs (edit), src-tauri/Cargo.toml (edit), src-tauri/tauri.conf.json (edit), src-tauri/capabilities/default.json (edit)
